...
public class SystemCameraActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == REQUEST_CODE_TAKE_PHOTO && resultCode == RESULT_OK) {
            ...
            Bitmap bm = (Bitmap)extra.get("data");
            imageView.setImageBitmap(bm);
        } else if (requestCode == REQUEST_CODE_TAKE_PHOTO_PATH && resultCode == RESULT_OK) {
            // todo 1.4 通过图片地址构造 bitmap
            int tw = imageView.getWidth();
            int th = imageView.getHeight();
            BitmapFactory.Options option = new BitmapFactory.Options();
            option.inJustDecodeBounds = true;
            BitmapFactory.decodeFile(takeImagePath,option);
            int pw = option.outWidth;
            int ph = option.outHeight;
            int scalef = Math.min(pw/tw,ph/th);
            option.inJustDecodeBounds =false;
            option.inSampleSize = scalef;
            Bitmap bm = BitmapFactory.decodeFile(takeImagePath,option);
            imageView.setImageBitmap(bm);
            // todo 1.5 注意图片大小
        }
    }
